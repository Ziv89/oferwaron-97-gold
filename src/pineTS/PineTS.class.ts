import { Context } from './Context.class.ts';
import { IProvider } from './marketdata/IProvider.ts';
import { transpile } from './transpiler/transpiler.ts';



globalThis.ta = globalThis.ta || {};
globalThis.math = globalThis.math || Math;
globalThis.indicator = globalThis.indicator || (() => { });
globalThis.strategyEntry = globalThis.strategyEntry || (() => { });
globalThis.strategyExit = globalThis.strategyExit || (() => { });
globalThis.labelNew = globalThis.labelNew || (() => { });
globalThis.labelDelete = globalThis.labelDelete || (() => { });

export class PineProcessor {
  public _readyPromise: Promise<any> | null = null;
  ...
  private async loadMarketData(
    ...
    if (Array.isArray(source)) {
      ...
      return (source as IProvider).getMarketData(
  ...
  public async run(
    pineTSCode: string,
    n?: number,
    useTACache: boolean = false
  ): Promise<Context> {
    await this.ready();

    if (!n) n = this._periods;
    if (n > this._periods) n = this._periods;
    ...
    // Preprocess Pine code into JS
    // const jsCode = pineToJS(pineTSCode);

    // const wrappedCode = `
    //   () => {
    //     ${jsCode}
    //     return {
    //       ema9, ema18, bull_bias, bear_bias, prev_close, diff_close, _oo
    //     };
    //   }
    // `;
...
console.log(transpiledFn.toString());
    ...
    if (typeof transpiledFn !== 'function') {
      throw new Error('Transpilation did not return a function');
    }
...
export async function PineTSRunner(
  pineCode: string,
  marketData: any[],
  source: IProvider | any[] = marketData,
  tickerId: string = 'DEFAULT_TICKER',
  timeframe: string = '1d',
  limit: number = marketData.length,
  sDate: number = Date.now(),
  eDate: number = Date.now()
) {
  const processor = new PineProcessor(
    source,
    tickerId,
    timeframe,
    limit,
  ...
  const context = await processor.run(pineCode);
