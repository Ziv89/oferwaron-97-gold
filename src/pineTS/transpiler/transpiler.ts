...
import ScopeManager from './ScopeManager.class.ts';
import * as babelParser from '@babel/parser';
import preprocessPineScript from './preprocessPineScript';

import { Statement } from 'estree'; // if you're using @types/estree
...
scopeManager.addContextBoundVar(param.name);
        ...
        if (isArrowFunction) {
            ...
            walk.recursive(decl.init.body, scopeManager, {
                ...
                },ExpressionStatement(node: any, state: ScopeManager, c: any) {
  if (node.expression.type === 'AssignmentExpression') {
    const transformed = transformAssignmentExpression(node.expression, state);
    if (transformed && Array.isArray(transformed)) {
      Object.assign(node, transformed[0]);
    }
  }
},
...
function transformAssignmentExpression(node: any, scopeManager: ScopeManager): any[] {

      if (node.left.type === 'ArrayPattern') {
    const tempVar = scopeManager.generateTempVar(); 
const assignments: any[] = [];
    ...
    // Destructure each item from temp var
    node.left.elements.forEach((element, index) => {
      if (element && element.type === 'Identifier') {
        const name = element.name;
        scopeManager.markDeclared(name);

        assignments.push({
          type: 'VariableDeclaration',
          kind: name.startsWith('_') ? 'let' : 'const',
          declarations: [{
            type: 'VariableDeclarator',
    ...
    if (node.left.type === 'Identifier') {
        ...
        const alreadyDeclared = scopeManager.isDeclared(varName);

        if (!alreadyDeclared) {
            scopeManager.markDeclared(varName); // Mark this variable as declared
            const kind = varName.startsWith('_') ? 'let' : 'const';
        ...
        // Otherwise, treat as reassignment
      return [{
      type: 'ExpressionStatement',
      expression: {
        type: 'AssignmentExpression',
...
const boundParamNames: string[] = [];
node.params.forEach((param: { type: string; name?: string }) => {
...
function preProcessContextBoundVars(ast: any, scopeManager: ScopeManager): void {
    walk.simple(ast, {
        VariableDeclaration(node: any) {
            node.declarations.forEach((decl: any) => {
                ...
                if (decl.id && decl.id.type === 'ObjectPattern' && decl.id.properties) {
    decl.id.properties.forEach((property: any) => {
        if (property.key?.name) {
            scopeManager.addContextBoundVar(property.key.name);
        }
  ...
  function transpile(fn: string | Function): Function {

//     function preprocessPineScript(code) {
//   return code
//     .replace(/\/\/\@version=5/g, '')                              // remove version comment
//     .replace(/strategy\(/g, 'indicator(')                           // unify strategy → indicator
//     .replace(/var\s+(float|int|label|color|bool)?\s*/g, 'let ')    // var float → let
//     .replace(/:=/g, '=')                                             // Pine-style assignment
//     .replace(/\bna\b/g, 'NaN')                                     // na → NaN
//     .replace(/\[(\w+),\s*(\w+),\s*_\]/g, '[$1, $2]');           // remove unused _ from destructuring
// }

    // let code = typeof fn === 'function' ? fn.toString() : fn;
    let code = typeof fn === 'function' ? fn.toString() : fn;

code = preprocessPineScript(code);
//     code = code
//   .replace(/\/\/@version=5/g, '')                      // remove comment header
//   .replace(/var\s+(float|int|label|bool|color)?\s*/g, 'let ')  // var float → let
//   .replace(/:=/g, '=')                                 // Pine-style assignment
//   .replace(/:=/g, '=')                                 // Pine-style assignment
//   .replace(/\bna\b/g, 'NaN'); 
  
// code = code.replace(
//   /^\s*let\s*\[\s*(\w+)\s*,\s*(\w+)\s*,\s*_\s*\]\s*=/gm,
//   (full, a, b) => `let [${a}, ${b}, __ignore] =`
// );
...
// Second pass: transform the code, including top-level assignments
for (const stmt of ast.body) {
  if (stmt.type === 'ExpressionStatement' && stmt.expression.type === 'AssignmentExpression') {
    const transformed = transformAssignmentExpression(stmt.expression, scopeManager);
    if (transformed?.length) {
      Object.assign(stmt, transformed[0]); // Replace with VariableDeclaration
      continue;
    }
  }

  // Walk and transform the rest
  walk.recursive(stmt, scopeManager, {
    ExpressionStatement(node: any, state: ScopeManager, c: any) {
      if (node.expression.type === 'AssignmentExpression') {
        const transformed = transformAssignmentExpression(node.expression, state);
        if (transformed?.length) Object.assign(node, transformed[0]);
      }
    },
    BlockStatement(node: any, state: ScopeManager, c: any) {
      node.body.forEach((stmt: any) => c(stmt, state));
...
// `;
    ...
    // const transpiledFn = compiled();
//       const transpiledFn = new Function(finalCode);
//  console.log(transpiledFn);
//   return transpiledFn();
    // return _wraperFunction(this);
